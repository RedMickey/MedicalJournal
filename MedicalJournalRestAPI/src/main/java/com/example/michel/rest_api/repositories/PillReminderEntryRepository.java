...
public interface PillReminderEntryRepository extends CrudRepository<PillReminderEntry, UUID> {
    ...
    @Modifying
    @Query(
            value = "UPDATE pill_reminder_entry SET synch_time = :timestamp, change_type = 3 WHERE _id_pill_reminder = :idPillReminder",
            nativeQuery = true)
    void updateAndMarkAsDeletedByPillReminderId(@Param("idPillReminder") UUID idPillReminder,
                                                @Param("timestamp") Timestamp synchronizationTimestamp);

    @Modifying
    @Query(
            value = "UPDATE pill_reminder_entry SET synch_time = :timestamp, change_type = 3 WHERE DATE(reminder_date) >= " +
                    "DATE(:reminderDate) and _id_pill_reminder = :idPillReminder and is_done = 0",
            nativeQuery = true)
